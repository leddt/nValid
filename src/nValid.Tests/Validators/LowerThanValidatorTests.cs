...
{
    public class LowerThanValidatorTests : TestBase
    {
        [Test]
        public void Should_be_valid_when_value_is_lower()
        {
            var validator = new LowerThanValidator<Person, int>(18);

            Assert.That(validator.Validate(null, 16), Is.True);
        }

        [Test]
        public void Should_be_invalid_when_value_is_equal()
        {
            var validator = new LowerThanValidator<Person, int>(18);

            Assert.That(validator.Validate(null, 18), Is.False);
        }
        ...
        {
            var validator = new LowerThanValidator<Person, int>(10);
